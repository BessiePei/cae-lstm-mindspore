...
def cae_lstm_eval(encoded):
    ...
    num_modes = cae_model_params["latent_size"]  # 此处为保留模态个数
    V_modes = V[:num_modes, :]
    ...
    for i in range(prediction_params["decoder_data_split"]):
        ...
        cae_lstm_predict[time_predict_start: time_predict_end] = \
            np.squeeze(np.dot(lstm_latent[time_predict_start: time_predict_end].asnumpy(), V_modes).reshape(-1, 128, 128))
    ...
    print("PodLstm prediction mean error: " + str(cae_lstm_error_mean))
...
if __name__ == "__main__":
    ...
    cae_latent = pod_eval(args.config_file_path, args.case)
